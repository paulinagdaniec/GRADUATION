...
st.write ("this is the first prototype of a font recommender system. Your input and feedback will be used for further developement. Thank you for your time and enjoy! 💙  ")
...
feels = ['modern', 'clean', 'open', 'informal', 'progressive', 'simple', 'minimal', 'contemporary', 'bold', 'creative',
         ...
         'expressive', 'elegant', 'romantic', 'feminine', 'technical', 'precise', 'retro', 'futuristic',
         'structured']
...
st.subheader(f"Most suitable typeface category is: {matched_character}")
...
weight_fonts_animation = load_lottiefile("weight_fonts_anim.json")

st_lottie(
    weight_fonts_animation, 
    loop = True, 
    height=400, 
    key="weights")
...
def save_to_csv(user,feels, text, font, liked, disliked, saved, feedback=False):
    filename = "user_input.csv"
    headers = ["User Name", "Text", "Selected Font", "Liked", "Disliked", "Saved"]
    mode = "a" if os.path.exists(filename) else "w"
    with open(filename, mode, newline="") as f:
        writer = csv.writer(f)
        if mode == "w":
            writer.writerow(headers)
        writer.writerow([user,feels, text, font, liked, disliked, saved])
...
if user_text:
    ...
    if liked or disliked:
        save_to_csv(user_name, selected_feels, user_text, font_name, liked, disliked, False, feedback=True)

    # Add a "Save" button to store the input text and font to a CSV file in the third column
    if col3.button("Save to library"):
        save_to_csv(user_name, selected_feels, user_text, font_name, False, False, True)


# Display the user's saved fonts

library = pd.read_csv("user_input2.csv")
...
if show_saved:
    ...
    if not user_preferences.empty:
        st.write(user_preferences)
    else:
        st.write("No saved fonts for this name.")
